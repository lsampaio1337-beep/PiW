        ...
        content += `
            <div onclick="window.setActiveItem('ball', ${idx})" style="text-align: center; width: 100%; cursor: pointer; border: 2px solid ${isActive ? '#2ecc71' : 'transparent'}; border-radius: 8px; padding: 5px; background: ${isActive ? 'rgba(46,204,113,0.2)' : 'transparent'}; box-sizing: border-box;">
                <div style="width: 100%; aspect-ratio: 1/1; margin: 0 auto; display: flex; align-items: center; justify-content: center;">
                    <img src="./Assets/Items/Balls/${b.name.replace(/Regular Pokeball/, "Pokeball").replace(/Great Pokeball/, "Greatball").replace(/Ultra Pokeball/, "Ultraball").replace(/Master Pokeball/, "Masterball")}.png" style="max-width: 100%; max-height: 100%; object-fit: contain;" onerror="this.src='./Assets/Extra/Spot.png'">
                </div>
        ...
        if (qty > 0) {
            ...
            content += `
                <div style="background: #333; border: 1px solid #555; border-radius: 8px; padding: 5%; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: space-between; aspect-ratio: 1 / 1.5; box-sizing: border-box; container-type: inline-size;">
                    <img src="./Assets/Items/Balls/${b.name.replace(/Regular Pokeball/, "Pokeball").replace(/Great Pokeball/, "Greatball").replace(/Ultra Pokeball/, "Ultraball").replace(/Master Pokeball/, "Masterball")}.png" onerror="this.src='./Assets/Extra/Spot.png'" style="width: 50%; max-height: 30%; object-fit: contain; margin-bottom: 5px;">
                    ...
                    <div style="margin-top: 10px;">
                        <input type="number" id="sell-qty-ball-${b.name.replace(/\s+/g, '-')}" value="1" min="1" max="${qty}" oninput="window.updateLocalSellPrice('ball', '${b.name}', ${sellPrice})" style="width: 80%; text-align: center; margin-bottom: 5px; font-size: 12px;">
                    ...
                    <div id="sell-total-ball-${b.name.replace(/\s+/g, '-')}" style="font-size: 14px; font-weight: bold; color: #f1c40f; margin-top: 5px;">Total: $${window.formatMarketPrice(sellPrice)}</div>
                    <button onclick="window.sellItem('${b.name}', 'pokeballs', ${sellPrice}, 'ball')" style="margin-top: 5px; padding: 5%; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer; width: 100%; font-size: 10cqw;">Sell</button>
                </div>
            `;
        }
    });
...
window.updateLocalSellPrice = function(type, itemName, sellPrice) {
    ...
    if (input && totalDiv) {
        ...
        if (qty > max) qty = max;
        input.value = qty;
        totalDiv.innerText = `Total: ${window.formatMarketPrice(qty * sellPrice)}`;
    }
};
...
window.sellItem = function(itemId, category, sellPrice, typePrefix) {
    ...
    if (state.backpack[category][itemId] >= qty) {
        ...
        if (toast) {
            toast.innerText = `Sold ${qty}x ${itemId} for ${earnings.toLocaleString()}!`;
