...
export function navigateToLocation(locationName) {
    ...
    if (locationName === "Professor Oak Lab") {
        ...
        let buttonHtml = '';
        if (gymConfig) {
            buttonHtml = `<button onclick="window.enterGymLobby('${lookupName}')" style="padding: 10px 20px; font-size: 16px; margin-top: 10px; cursor: pointer; background-color: #3498db; color: white; border: none; border-radius: 5px;">Battle Gym</button>`;
        }

        vGym.innerHTML = `
            <div id="gym-content-area" style="position: absolute; left: 33%; top: 50%; transform: translate(-50%, -50%); display: flex; flex-direction: column; align-items: center; justify-content: center;">
                <div style="background-color: rgba(0,0,0,0.8); padding: 30px; border-radius: 8px; text-align: center; color: white;">
                    <h2>${locationName}</h2>
                    ${buttonHtml}
                </div>
            </div>
