...
async function init() {
                    ...
                    if (state.isZzZMode) {
                        const html = `
                            <div style="text-align: center; padding: 20px;">
                                <p style="color: white; margin: 0 0 15px 0;">Do you want to collect the farm while Sleeping?</p>
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                const closeBtn = document.getElementById('btn-zzz-results-close-dynamic');
                                if (closeBtn) {
                                    closeBtn.onclick = () => {
                                        if(window.closeModal) window.closeModal();
                                    };
    ...
    bindBtn('btn-sleep', () => {
        if(!checkCombatLock()) {
            ...
            const html = `
                <div style="display: flex; flex-direction: column; gap: 15px; text-align: center; padding: 10px;">
                    ${showTutorial ? `
                    <div id="zzz-tutorial-section" style="background: rgba(255,255,255,0.05); border: 1px dashed #475569; border-radius: 8px; padding: 15px; margin-bottom: 10px; text-align: left;">
                    ...
                    <div style="display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,0.2); border-radius: 8px; padding: 20px; border: 1px solid #334155; gap: 20px;">
                        <div style="flex: 1; display: flex; justify-content: flex-end;">
                            <img src="Assets/Extra/Jigglypuff Dust.png" style="width: 150px; height: auto; filter: drop-shadow(0 0 10px rgba(255, 192, 203, 0.4));">
                        </div>
                    ...
                    <button id="btn-zzz-cheat-grains" style="margin-top: 15px; padding: 5px 10px; font-size: 11px; cursor: pointer; background: transparent; color: #94a3b8; border: 1px dashed #475569; border-radius: 4px; width: fit-content; align-self: center;">Add +10 grains</button>
                </div>
            `;
            showModal("ZzZ Mode", html);
            window.windowManager.toggleWindow('modal-content-box', true);
            ...
            setTimeout(() => {
                ...
                bindBtn('btn-zzz-cheat-grains', () => {
                    ...
                    const newGrains = state.stats.jigglypuffGrains;
                    const el = document.getElementById('zzz-current-grains');
                    if (el) el.innerText = newGrains;
                });
            }, 0);
        }
    });
